...
static bool get_system_time(){
    ...
    if(time_temp.tm_min == last_minute)
        return false;
        last_minute = time_temp.tm_min;
    if(time_temp.tm_hour < 10){
        sprintf(time_str,"0%d:%02d",time_temp.tm_hour,time_temp.tm_min);
    }else
    {
         sprintf(time_str,"%d:%02d",time_temp.tm_hour,time_temp.tm_min);
    }
    return false;
    */
}
...
static void refresh(lv_event_t* event){
    ...
    static int  last_ble_state = -1;
    static int  last_vol_value = -1;
    static bool last_alarm_on = false;
    if(device_state->wifi_connect_state != last_wifi_state){
        last_wifi_state = device_state->wifi_connect_state;
        if(last_wifi_state == WPA_WIFI_CONNECT)
    ...
    if(device_state->ble_mesh_state != last_ble_state){
        last_ble_state = device_state->ble_mesh_state;
        if(last_ble_state == CONNECT)
        lv_img_set_src(ble_img,GET_IMAGE_PATH("icon_ble_connect.png"));
    else
        lv_img_set_src(ble_img,GET_IMAGE_PATH("icon_ble_disconnect.png"));
      }
    if(device_state->volume_value != last_vol_value){
        last_vol_value = device_state->volume_value;
        if(last_vol_value == 0)
    ...
    bool alarm_on = (device_state->is_open_type1 || device_state->is_open_type2);
...
static void init_item_view(lv_obj_t *parent,TIME_SHOW_TYPE_E type){
    if(type == TIME_TYPE_1){
        ...
        lv_obj_align(time_obj,LV_ALIGN_TOP_MID,0,70);
        ...
        lv_obj_align(clock,LV_ALIGN_TOP_MID,0,70);
    }
}

static lv_obj_t * init_info_view(lv_obj_t *parent){
    lv_obj_t * cont = lv_obj_create(parent);
    lv_obj_set_size(cont, LV_PCT(25), LV_PCT(100));
    lv_obj_center(cont);
...
static lv_obj_t *init_scroll_view(){
    ...
    lv_obj_set_pos(scroll_view,0,-LV_VER_RES);
...
void init_page_main(void)
{
    ...
    if(page_type == TIME_TYPE_2){
        ...
        lv_obj_align(gif_obj,LV_ALIGN_LEFT_MID,-30,-30);
    ...
    lv_obj_align_to(info_obj,user_avatar_obj,LV_ALIGN_OUT_RIGHT_MID,40,150);
